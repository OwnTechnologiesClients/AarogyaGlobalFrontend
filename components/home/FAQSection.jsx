...
// Custom trigger for FAQ accordion, using plus/minus icons
const FAQAccordionTrigger = forwardRef(({ children, open, ...props }, ref) => (
	<AccordionTriggerNoIcon
		ref={ref}
		{...props}
		className={cn(
			...
			<div className="w-full flex flex-col md:flex-row gap-12 z-10 px-20">
				...
				<div className="flex-1 flex flex-col justify-center gap-6">
					...
					<div className="flex items-center gap-6 mt-4">
						<Button size="lg" className="bg-green-500 hover:bg-green-600 text-white font-semibold px-8 py-4 rounded-xl shadow-lg flex items-center gap-2 transition-all duration-200">
							Contact Us Now
				...
				<div className="flex-1 flex flex-col gap-6">
					<Accordion type="single" collapsible value={open} onValueChange={setOpen}>
						{faqData.map((faq, idx) => {
							...
							return (
								<AccordionItem key={idx} value={`faq-${idx}`} className="bg-transparent border-none p-0 mb-6">
									<FAQAccordionTrigger open={isOpen}>
										{`${idx + 1}.${faq.question}`}
									</FAQAccordionTrigger>
									<AccordionContent forceMount className={cn(
										"px-8 pb-8 pt-0 text-gray-500 text-base bg-white rounded-b-2xl border-x border-b border-green-400 animate-fade-in",
										isOpen ? "block" : "hidden"
									)}>
										{faq.answer}
									</AccordionContent>
