...
const FilterBar = () => {
  ...
  return (
    <div className="w-full flex justify-center px-4 md:px-6 lg:px-8 relative z-0">
      {/* Main Filter Container */}
      <div className="bg-white rounded-xl md:rounded-2xl shadow-2xl border border-gray-100 w-full max-w-5xl relative mt-4 md:mt-0 z-0">

        {/* Toggle Buttons - Positioned at top center */}
        <div className="absolute -top-4 md:-top-6 left-1/2 transform -translate-x-1/2 z-[5]">
          <div className="flex gap-0.5 md:gap-1 bg-white rounded-lg md:rounded-xl p-0.5 md:p-1 shadow-lg border border-gray-200">
            {filters.toggles.map((toggle) => (
              <button
                key={toggle.value}
                className={`px-3 md:px-6 py-2 md:py-2.5 rounded-md md:rounded-lg font-semibold transition-all duration-200 text-xs md:text-sm focus:outline-none whitespace-nowrap ${
          ...
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 lg:gap-6">
            ...
            <div className="flex flex-1 gap-3 flex-col md:flex-row w-full lg:w-auto">
              {filters.filters.map((filter) => (
                ...
                >
                  <SelectTrigger className="w-full md:w-48 lg:w-52 h-11 bg-gray-50 text-gray-700 rounded-lg border border-gray-200 shadow-sm hover:border-[#04CE78]/50 focus:ring-2 focus:ring-[#04CE78]/30 focus:border-[#04CE78] transition-all duration-200">
            ...
            <div className="flex gap-3 w-full lg:w-auto flex-col md:flex-row">
              {filters.actions.map((action) => (
                <Button
                  ...
                  className={`font-semibold px-6 py-2.5 h-11 rounded-lg shadow-md hover:shadow-lg flex items-center gap-2 text-sm w-full md:w-auto transition-all duration-200 ${
