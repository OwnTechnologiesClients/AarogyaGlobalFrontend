    ...
    const individualTreatmentData = {
      treatment: {
        ...
        bestHospitals: {
          ...
          hospitals: (treatment.hospitals || treatment.topHospitals || []).map(hospitalId => {
        ...
        topDoctors: {
          ...
          doctors: (() => {
            ...
            if (doctorIds.length > 0) {
              ...
              // Fallback to specialty doctors if no specific doctors are mapped
              console.log(`getIndividualTreatmentData: No specific doctors mapped for treatment ${treatment.id}, using specialty doctors`);
              const specialtyDoctors = this.getDoctorsBySpecialty(specialty.slug) || [];

              if (specialtyDoctors.length > 0) {
                return specialtyDoctors;
              } else {
                // If no specialty doctors, try to find doctors from related specialties or global doctors
                console.log(`getIndividualTreatmentData: No specialty doctors found, looking for related doctors`);

                // Get all global doctors and filter by relevance
                const allDoctors = this.getAllGlobalDoctors();
                if (allDoctors.length > 0) {
                  // Return first few doctors as a fallback
