...
class DataService {
  ...
  async normalizeHospital(hospital) {
    ...
    const ratingValue = typeof hospital.rating === 'object'
      ? (hospital.rating.userScore ?? hospital.rating.googleRating ?? null)
      : hospital.rating;
    ...
    if (Array.isArray(hospital.doctors) && hospital.doctors.length > 0) {
      ...
      if (typeof first === 'string') {
        ...
        embeddedDoctors = hospital.doctors.map(d => this.normalizeDoctor(d));
      }
    }

    return {
      ...hospital,
      rating: ratingValue !== null ? ratingValue : hospital.rating,
      displayImage,
  ...
  */
  ...
  async getDoctorsPaginated(params = {}) {
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('getDoctorsPaginated:', normalized.pagination);
      }
