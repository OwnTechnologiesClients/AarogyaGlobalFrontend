...
class DataService {
  ...
  */
  ...
  async getDoctorsPaginated(params = {}) {
    try {
      const { page = 1, limit = 6, specialty, category, location, search, hospitalId } = params;
      ...
      const resolvedTotal = (() => {
        const apiTotal = response?.total ?? response?.totalDocs ?? response?.pagination?.total;
      ...
      const resolvedTotalPages = (() => {
        const apiTotalPages = response?.totalPages ?? response?.pagination?.totalPages;
        if (typeof apiTotalPages === 'number' && !Number.isNaN(apiTotalPages) && apiTotalPages > 0) {
          return apiTotalPages;
        }
        if (resolvedLimit > 0) {
          return Math.max(1, Math.ceil(resolvedTotal / resolvedLimit));
        }
        return 1;
      })();
