...
class SupportApiService {
  ...
  /**
   * Send message to an existing support ticket
   * @param {Object} messageData - Message data
   * @param {string} messageData.ticketId - Ticket ID
   * @param {string} messageData.message - Message content
   * @param {string} messageData.userId - User ID
   * @param {File} messageData.attachment - Optional attachment file
   * @returns {Promise} API response
   */
  async sendMessage(messageData) {
    ...
    return this.apiService.request(`/support/message/${messageData.ticketId}`, {
      method: 'POST',
      includeAuth: true,
      data: formData,
      headers: {
